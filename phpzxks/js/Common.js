function formcheck(){
  if (document.NextPage.Page.value == "" )
        {
        alert("请输入你想要的页数!");
        document.NextPage.Page.focus();//校验输入框输入值页面不会跳
        return false;
        }
...
{
var n=document.getElementById(f).style.display.indexOf("block");//non变block
...
function CancelCheckAll(f,n)//取消全选
{
    for (var i=0;i<f.elements.length;i++)
	{
	//alert(n);
